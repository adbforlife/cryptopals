import socketserver
import os
from cryptools import *
import random
import hashlib


n = 
g = 2
k = 3
I = b'adbforlife2018@gmail.com'
p = b'obfuscated'

def sha256(m):
    hash_obj = hashlib.sha256()
    hash_obj.update(m)
    return hash_obj.digest()

def hmac_sha256(k,m):
    opad = bytes([0x5c for _ in range(64)])
    ipad = bytes([0x36 for _ in range(64)])
    if len(k) > 64:
        k = sha256(k)
    else:
        k += bytes([0 for _ in range(64 - len(k))])
    return sha256(xor(k, opad) + sha256(xor(k, ipad) + m))

def perform_client(A, b, u, salt, password):
    x = int.from_bytes(sha256(salt + password), 'big')
    v = pow(g,x,n)
    s = pow(A * pow(v,u,n), b, n)
    k = sha256(s.to_bytes(192, 'big'))
    return hmac_sha256(k, salt)

def crack_password(A, b, u, salt, expected):
    passwords = open('/home/adb/ctf/rockyou.txt', 'rb').read().split(b'\n')
    for password in passwords:
        if perform_client(A,b,u,salt,password) == expected:
            print(f"SUCCESS {password}")
            return password
        else:
            print(f"FAILED {password}")
    return None
    

class MyTCPHandler(socketserver.StreamRequestHandler):

    def handle(self):
        # Prep
        salt = os.urandom(16)
        xH = sha256(salt + p)
        x = int.from_bytes(xH, 'big')
        v = pow(g,x,n)

        # Get client info for DH about A
        I = self.rfile.readline().strip()
        A = int(self.rfile.readline().strip())

        # send salt and B
        b = random.randint(1, n-1)
        B = pow(g,b,n)
        self.wfile.write(salt)
        self.wfile.write(str(B).encode() + b'\n')

        # compute uH
        uH = sha256(A.to_bytes(192, 'big') + B.to_bytes(192, 'big'))
        u = int.from_bytes(uH, 'big')

        # validation
        s = pow(A * pow(v,u,n), b, n)
        k = sha256(s.to_bytes(192, 'big'))
        res = hmac_sha256(k, salt)
        received = self.rfile.read(32)
        
        real_p = crack_password(A, b, u, salt, received)

        if real_p == b'iloveyou':
          self.wfile.write(b'OK\n')
        else:
          self.wfile.write(b'You are cheating\n')

if __name__ == "__main__":
    HOST, PORT = "localhost", 15213

    # Create the server, binding to localhost on port 9999
    with socketserver.TCPServer((HOST, PORT), MyTCPHandler) as server:
        # Activate the server; this will keep running until you
        # interrupt the program with Ctrl-C
        server.serve_forever()
